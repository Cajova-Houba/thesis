...
Identifikátor získaný v předchozím kroku lze použít k volání REST služby pro porovnávání. 

\paragraph{Výsledky porovnání}
TODO: stručně výsledek + rozebrání

\section{Testování}

- testování skrze REST API
- pomocí nástroje Postman (Collection Runner)
- princip: několik verzí jednoho API -> testování křížem
- testy byly provedeny pro každý typ služby jež moduly indexují
- detaily testů jsou rozvedeny v následujících sekcích

\subsection{Server v Jave}
\begin{itemize}
	\item metadata získaná z implementace
	\item postaveno na Jersey (todo: verze)
	\item REST API
	\item alespoň obrázek/raml api?
	\item 2 verze otestované křížem
\end{itemize}

Celkem jsou testovány dvě verze služby. První z nich obsahuje základní implementaci služby. Druhá verze (odvozená od první) pak mění datový typ parametru u jednoho z endpointů z \textit{Long} na \textit{Number}. V tabulce \ref{tab:jersey-cmp-res} s výsledky jsou tyto verze označovány \textit{v1} a \textit{v2}.
...
V tomto testovacím scénáři jsou použity 4 verze jedné služby. První verze označovaná \textit{v1} je základní a všechny následující verze jsou od této odvozené a pouze ji modifikují. Přehled změn je uveden v tabulce \ref{tab:json-wsp-diffs}, jednotlivé verze jsou v ní označeny \textit{v1} až \textit{v4} a sloupec 'Popis změny' vždy udává rozdíl mezi základní a danou verzí. Datový typ \textit{User}, jež je modifikovaný ve verzi \textit{v2} je definovaný přímo v popisném souboru.
