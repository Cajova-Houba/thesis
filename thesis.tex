...
Webové služby jsou v dnešní době stále populárnější součást vývoje aplikací a existuje spousta poskytovatelů, kteří skrze tyto nabízejí možnost řízené manipulace s jejich daty (Facebook, Twitter), integraci aplikací (Trello) nebo výpočetní výkon (AWS, Google). Webové služby také tvoří nedílnou součást vnitro-podnikové a mezi-podnikové komunikace v podobě ERP\footnote{Enterprise Resource Planning} a EDI\footnote{Electronic Data Interchange} systémů.
 
% nastínění problémů s kompatibilitou služeb
% todo: formulace
Aby propojení služeb fungovalo správně a bylo možné je pohodlně škálovat, je nutná znalost operací nebo zdrojů, poskytovaných službou, a také způsobu, kterým služba komunikuje. Všechny tyto informace jsou popsány v rozhraní poskytnutém službou na jehož správnosti závisí úspěch celé komunikace. Případné změny implementace služby, jež přímo naruší kontrakt daný rozhraním, mohou na straně konzumenta služby zapříčinit problémy potenciálně vedoucí až k výpadku aplikace, která službu využívá. 

Rozhraní také odděluje popis funkcionality od konkrétní implementace služby, kterou může dodávat mnoho poskytovatelů a konzumující aplikace mezi nimi mohou přepínat ať už na základě přechodu mezi regiony nebo dočasné nedostupnosti. Aby nedošlo k chybám při tomto přepnutí, je nutné kontrolovat vzájemnou kompatibilitu těchto služeb.
