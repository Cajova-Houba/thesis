...
\label{sec:diff-info}
Rozdíl webových služeb se nedá vyjádřit stejné/nestejné. Proč? Pro klienta to nemusí být dostatečná informace (může ho zajímat co přesně není stejné), navíc i webové služby, které nejsou stejné mohou být z pohledu klienta kompatibilní. Proto je potřeba rozdíly mezi službami reprezentovat něčím trochu chytřejším než např boolean hodnotou.

Článek \cite{brada2006diff} zabývající se možnostmi evaluace kompatibility komponent na základě relace subtypingu, nahlíží na komponentu skrze její rozhraní jako na datový typ a popisuje odlišnosti v různých úrovních kontraktu (metoda, parametr, ...). Na základě těchto odlišností (a relace subtypingu) je pak určena míra kompatibility dvou komponent. Protože je webová služba v CRCE reprezentována komponentou a na její rozhraní se dá také nahlížet jako na kontrakt datového typu, použil jsem tento přístup k reprezentaci výsledků porovnání v mé práci.  
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|c|}
		\hline
		Název úrovně & Váha & Priorita & Popis \\
		\hline
		\hline
		None & NON & 1 & $a = b$ \\
		\hline
...
Způsob vyhodnocení rozdílů dvou webových služeb společně s významem jednotlivých úrovní \textit{Difference} a jejich vah pro klienta je popsán na konci této kapitoly. 
