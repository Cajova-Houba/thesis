...
\subsection{Záznamy}
 ...
 - A record is a collection of fields, possibly of different data types, typically in fixed number and sequence \cite{felleisen2001}
 - v C např \textit{struct}, v Pascalu \textit{record}
 - Java záznamy neobsahuje
 - vlastní datové typy v XSD definované elementem \textit{complexType} jsou záznamy
...
Relace subtypingu je pro záznamové datové typy $S = \{k_1:S_1 ... k_m:S_m\}$ a $T = \{l_1:T_1 ... l_n:T_n\}$ definována pomocí dvou pravidel: subtyping do šířky a subtyping do hloubky. Subtyping do šířky říká, že typ s více záznamy je subtyp typu s méně záznamy, musí tedy platit $n \leq m$. Pravidlo subtypingu do hloubky se zabývá typem jednotlivých položek a říká, že mezi položkami na stejné pozici musí platit relace subtypingu, tedy pro každé $i : S_i <: T_i$.

Uvažujeme-li na příklad typy $S$ a $T$ definované jako:

\begin{equation}
\centering
	S = \{a:int, b:boolean, c:char\}, 
	T = \{a:long\}
\end{equation}
...
\end{equation} 
 ...
 však $S <: T$ neplatí, protože $\overline{int <: short}$ a $\overline{boolean <: char}$ a i když je dodrženo pravidlo "do šířky", není dodrženo pravidlo "do hloubky".

\subsubsection{Objekty} 	

 - obdobné jako u záznamů
 - u metod navíc aplikován princip kontravariance
...
\subsection{Statická}
 ...
 - datový typ proměnné je známý v době času překladu
 - 

\subsection{Dynamická}

 - datový typ proměnné je známý až v čase běhu
...
Testovací scénář je zaměřen na služby popsané formátem JSON-WSP. Vstupní metadata byla získána indexováním čtyř verzí popisu uměle vytvořeného rozhraní založeného na příkladu převzatém z Wikipedie\footnote{https://en.wikipedia.org/wiki/JSON-WSP}. Odlišnosti mezi jednotlivými verzemi, označenými \textit{v1} až \textit{v4}, jsou rozepsány v tabulce \ref{tab:json-wsp-diffs}. Všechny verze popisů rozhraní služby jsou umístěny v příloze tohoto textu.
...
\chapter{Popisy rozhraní testovaných služeb}

\section*{REST služba založená na Jersey}

\subsection*{Verze 1}

\subsection*{Verze 2}

\section*{Služba popsaná JSON-WSP}
...
\section*{Popis služby FuelEconomy}
...
\section*{Popis služby STAG-Číselníky}
